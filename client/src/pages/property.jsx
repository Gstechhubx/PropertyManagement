...
export default function Property() {
    ...
    const handleEditHouse = (e) => {
        ...
        axios.put(`http://localhost:9000/property/${property_id}/edithouse/${e.target.getAttribute("data-id")}`, formData)
            .then(res => {
                console.log(res.data);
                setHouses(res.data);
            })
            ...
            <div className='pt-4 h-[100vh] max-h-max bg-gray-100'>
                ...
                (
                    <div key={house._id} className='flex flex-col space-y-5 p-5  m-5 bg-white shadow-md transition-all duration-400 ease-in-out hover:shadow-xl rounded-lg'>
                        ...
                        <span>
                            <button data-id={house._id} className='bg-blue-500 border hover:bg-white transition-colors ease-in-out duration-250 hover:text-black border-blue-300 w-[5rem] text-white rounded-lg p-2' onClick={handleEditHouse}>Edit</button>
